...
#ifdef ANDROID
    /* PORT: no desktop to show a box on; a blocking dialog here left the process alive
       and silent on the headset. Abort instead, so the failure leaves a tombstone. */
    abort();
#else
...
#else
    /* tv_nsec must be less than one second. The watchdog sleeps for a
     * second, which previously returned EINVAL immediately and busy-spun. */
    struct timespec spec = { hns / 10000000, (hns % 10000000) * 100 };
