...
#ifndef CROUCHMODE_TOGGLE_ANALOG
#define CROUCHMODE_TOGGLE_ANALOG 0
#endif

#ifndef CROSSHAIR_HEALTH_ON_WHITE
#define CROSSHAIR_HEALTH_ON_WHITE 0
#endif

#ifndef HUDCENTER_NORMAL
#define HUDCENTER_NORMAL 0
#define HUDCENTER_WIDE 1
#endif

/*
 * GoldenEye's title stage. 0x25 was Perfect Dark's, and in GoldenEye's level
 * table that number is an ordinary mission, so the port booted straight into
 * a level load instead of the menus. LEVELID_TITLE in bondconstants.h.
 */
#ifndef STAGE_TITLE
#define STAGE_TITLE 90
#endif

#ifndef STAGE_CITRAINING
...
static struct extplayerconfig g_PlayerExtCfg[MAX_PLAYERS];
static s32 g_HudCenter = 0;
u32 g_HudAlignModeL = 0;
u32 g_HudAlignModeR = 0;
static s32 g_MenuMouseControl = 0;
static f32 g_ViShakeIntensityMult = 1.0f;
static s32 g_MusicDisableMpDeath = 0;
static s32 g_BgunGeMuzzleFlashes = 0;
static s32 g_MaxExplosions = 64;
static s32 g_ValidGbcRomFound = 0;
static s32 g_SndDisabled = 0;
...
Java_com_gevr_port_MainActivity_nativeInit(JNIEnv* env, jobject thiz, jstring dataPath) {
    ...
    sysLogPrintf(LOG_NOTE, "GEVR Android native init complete, data path: %s", g_data_path);
}

int pd_main(int argc, const char **argv);

JNIEXPORT void JNICALL
Java_com_gevr_port_MainActivity_nativeStartGame(JNIEnv* env, jobject thiz) {
    __android_log_print(ANDROID_LOG_INFO, "GEVR", "nativeStartGame called");
    if (g_initialized) {
        char* argv[] = {"gevr", NULL};
...
JNIEXPORT void JNICALL
Java_com_gevr_port_MainActivity_nativeAudioPause(JNIEnv* env, jobject thiz) {
    extern void audioPause(void);
    audioPause();
}

JNIEXPORT void JNICALL
Java_com_gevr_port_MainActivity_nativeAudioResume(JNIEnv* env, jobject thiz) {
    extern void audioResume(void);
    audioResume();
}
...
#endif
{
   ...
   // g_SkipIntro = true; // SKIP INTRO
    ...
    if (g_StageNum == STAGE_TITLE && (sysArgCheck("--skip-intro") || g_SkipIntro)) {
        ...
        g_StageNum = STAGE_TITLE; // boot normal !!
    }
