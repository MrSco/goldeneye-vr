...
static inline void romdataLoadRom(void)
{
    ...
    if (!g_RomFile) {
        ...
        for (size_t i = 0; i < sizeof(fallbacks) / sizeof(fallbacks[0]); i++) {
            g_RomFile = fsFileLoad(fallbacks[i], &g_RomFileSize);
            if (g_RomFile) {
                romName = fallbacks[i];
                sysLogPrintf(LOG_NOTE, "Found ROM under fallback name: %s", romName);
                break;
            }
        }
    }
    ...
    if (!g_RomFile) {
        sysFatalError("Could not open ROM file %s.\nEnsure that it is in the %s directory or /sdcard/GEVR/.", romName, fsFullPath(""));
    }
