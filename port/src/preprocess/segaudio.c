...
static u32 convertAudioAdpcmBook(u8 *dst, u32 dstpos, u8 *src, u32 srcpos)
{
	...
	s32 order = (s32)PD_BE32(n64_book->order);
	s32 npredictors = (s32)PD_BE32(n64_book->npredictors);
	/* order * npredictors * ADPCMVSIZE (8) coefficients — variable tail past book[1] */
	s32 ncoeffs = order * npredictors * 8;
	s32 i;

	if (order < 1 || order > 8 || npredictors < 1 || npredictors > 16 || ncoeffs < 1 || ncoeffs > 1024) {
		sysFatalError("ALADPCMBook at src 0x%x: bad order=%d npredictors=%d", srcpos, order, npredictors);
	}

	host_book->order = order;
	host_book->npredictors = npredictors;
	for (i = 0; i < ncoeffs; i++) {
		host_book->book[i] = PD_BE16(n64_book->book[i]);
	}

	/* header (two s32) + coefficients; keep 8-byte alignment for the next item */
	dstpos += 8 + (u32)ncoeffs * sizeof(s16);
	return PD_ALIGN(dstpos, 8);
}
...
static u32 convertAudioWaveTable(u8 *dst, u32 dstpos, u8 *src, u32 srcpos)
{
	...
	/* Leave flags clear so alBnkfNew always relocates offsets → host pointers. */
	host_wavetable->flags = 0;
...
u8 *preprocessALBankFile(u8 *src, u32 size, u32 *outSize)
{
	...
	const u32 dstlen = size * 4; /* pointers widen + full ADPCM books */
...
{
	/* Perfect Dark sequence table; GoldenEye uses RareALSeqBankFile instead. */
	(void)data;
	(void)size;
	(void)outSize;
	return NULL;
