...
/*
 * SIMD opt-outs for the Quest build.
 *
 * Both were added while chasing "the Rare logo sounds wrong", on the theory
 * that NEON decode differed from aspMain. The real cause turned out to be a
 * sound-id off-by-one (SND_SOUND_INDEX_BIAS), so the theory was wrong.
 *
 * tools/gevr_mixer_ab then ran both paths over every ADPCM wave in the sfx
 * and instrument banks on a Quest 3 - 2,120,512 samples, all 13 shift values,
 * npredictors 1 and 4:
 *
 *   aADPCMdec  bit-exact (0 differing samples), NEON 2.75x faster -> NEON on.
 *   aMix       differs by <= 1 LSB on ~29% of samples, NEON 3.0x faster, but
 *              the absolute cost is microseconds per audio frame either way.
 *              Scalar accumulates (out * 0x7fff + in * gain + 0x4000) >> 15
 *              with one clamp; NEON does out + round(in * gain) with two
 *              saturations; mupen's RSP model does neither exactly. With no
 *              reference to call a winner and nothing to gain, leave it.
 */
#ifndef GEVR_SCALAR_ADPCM
#define GEVR_SCALAR_ADPCM 0
#endif

#ifndef GEVR_SCALAR_MIX
#ifdef GEVR
#define GEVR_SCALAR_MIX 1
#else
#define GEVR_SCALAR_MIX 0
#endif
#endif
...
#ifdef GEVR
/* Raw PCM in the cartridge remains big-endian; ADPCM loads stay byte-oriented. */
void aLoadBufferRaw16Impl(const void *source_addr) {
 const uint8_t *src = source_addr;
 int16_t *dst = BUF_S16(rspa.in);
 int count = ROUND_UP_8(rspa.nbytes) / 2;
 for (int i = 0; i < count; i++) {
  dst[i] = (int16_t)(((uint16_t)src[2*i] << 8) | src[2*i+1]);
 }
}
#endif
...
void aLoadADPCMImpl(int num_entries_times_16, const int16_t *book_source_addr) {
 if (!book_source_addr || num_entries_times_16 <= 0 ||
     (size_t)num_entries_times_16 > sizeof(rspa.adpcm_table)) {
  sysFatalError("invalid ADPCM book size %d", num_entries_times_16);
 }
 memcpy(rspa.adpcm_table, book_source_addr, (size_t)num_entries_times_16);
}
...
void aSetVolumeImpl(uint8_t flags, int16_t v, int16_t t, int16_t r) {
 ...
 if (flags & A_LEFT) {
 rspa.target[0] = v;
 /* Rare _getRate: signed 16.16 additive delta (not SM64's unsigned multiplier). */
 rspa.rate[0] = ((int32_t)t << 16) | (uint16_t)r;
 } else {
 rspa.target[1] = v;
 rspa.rate[1] = ((int32_t)t << 16) | (uint16_t)r;
 }
 }
}
#endif
...
void aADPCMdecImpl(uint8_t flags, ADPCM_STATE state) {
 /* GEVR_SCALAR_ADPCM forces the scalar nibble<<shift path. It was added on a
  * since-disproved theory about the Rare logo; see the knob at the top. */
#if HAS_SSE41 && !GEVR_SCALAR_ADPCM
...
#elif HAS_NEON && !GEVR_SCALAR_ADPCM
...
void aEnvMixerImpl(uint8_t flags, ENVMIX_STATE state) {
 ...
 _Static_assert(sizeof(saved) <= sizeof(ENVMIX_STATE), "GE envelope state too large");
 if (flags & A_INIT) {
  for (int c = 0; c < 2; c++) {
   saved.volume[c] = (int32_t)rspa.vol[c] << 16;
   /* rate/8 — same as mupen (trunc toward 0) and getv (>> 3). */
   saved.step[c] = rspa.rate[c] / 8;
   saved.target[c] = rspa.target[c];
  }
  saved.dry = rspa.vol_dry;
  saved.wet = rspa.vol_wet;
 } else {
  memcpy(&saved, state, sizeof(saved));
 }
 {
  const int32_t tgt[2] = {
   (int32_t)saved.target[0] << 16,
   (int32_t)saved.target[1] << 16,
  };
  const int nsamples = nbytes / 2;
  for (int i = 0; i < nsamples; i++) {
   int16_t sample = in[i];
   int16_t vol[2];
   for (int c = 0; c < 2; c++) {
    saved.volume[c] += saved.step[c];
    if (saved.step[c] <= 0) {
     if (saved.volume[c] <= tgt[c]) {
      saved.volume[c] = tgt[c];
      saved.step[c] = 0;
     }
    } else if (saved.volume[c] >= tgt[c]) {
     saved.volume[c] = tgt[c];
     saved.step[c] = 0;
    }
    vol[c] = (int16_t)(saved.volume[c] >> 16);
   }
   {
    ...
    if (flags & A_AUX) {
     int16_t gain_wet_l = clamp16(((int32_t)vol[0] * saved.wet + 0x4000) >> 15);
     int16_t gain_wet_r = clamp16(((int32_t)vol[1] * saved.wet + 0x4000) >> 15);
     wet[0][i] = clamp16(wet[0][i] + (((int32_t)sample * gain_wet_l) >> 15));
     wet[1][i] = clamp16(wet[1][i] + (((int32_t)sample * gain_wet_r) >> 15));
    }
   }
  }
 }
...
#if HAS_SSE41 && !GEVR_SCALAR_MIX
 __m128i gain_vec = _mm_set1_epi16(gain);
#endif
#if !HAS_NEON || GEVR_SCALAR_MIX
 int i;
 int32_t sample;
#endif
...
/*
 * Classic aspMain aPoleFilter (A_POLEF). Coefficients were loaded into
 * rspa.adpcm_table by the preceding aLoadADPCM(32, fccoef) from
 * _filterBuffer — same layout as mupen64plus alist_polef / GE audio HLE.
 * In-place (in == out) is the normal reverb call pattern.
 */
static int32_t polef_rdot(size_t n, const int16_t *x, const int16_t *y) {
 int32_t accu = 0;
 y += n;
 while (n != 0) {
...
void aPoleFilterImpl(uint8_t flags, int16_t gain, POLEF_STATE state) {
 ...
 /* Flat view of the 16 coefs just loaded: h1[8] then h2[8]. */
 const int16_t *h1 = &rspa.adpcm_table[0][0][0];
 const int16_t *h2 = h1 + 8;
 int16_t h2_before[8];
 int16_t h2_scaled[8];
 int16_t l1, l2;
 int i;
 ...
 for (i = 0; i < 8; i++) {
  h2_before[i] = h2[i];
  /* Local copy — do not mutate the shared ADPCM/coef table. */
  h2_scaled[i] = (int16_t)(((int32_t)h2[i] * (int32_t)gain) >> 14);
 }
 ...
 while (nbytes > 0) {
  ...
  for (i = 0; i < 8; i++) {
   int32_t accu = (int32_t)frame[i] * (int32_t)gain;
   accu += (int32_t)h1[i] * (int32_t)l1;
   accu += (int32_t)h2_before[i] * (int32_t)l2;
   accu += polef_rdot((size_t)i, h2_scaled, frame);
 ...
 /* Persist last four outputs (POLEF_STATE[4]). */
 state[0] = dst[-4];
 state[1] = dst[-3];
 state[2] = dst[-2];
 state[3] = dst[-1];
