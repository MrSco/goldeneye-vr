 ...
 */
/* The first level expands the argument, so that a macro defined as nothing
   reads as empty. That expansion can introduce commas (a command macro
   expands to comma-separated data), so the second level absorbs the extra
   arguments and tests only the first token. */
