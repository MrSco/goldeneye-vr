...
/*
 * PORT: there is no kseg0/kseg1 window on the host. A physical address and
 * a virtual one are the same pointer-width value, so these are identities;
 * the cartridge versions masked or set the top bits of a 32-bit word, which
 * would cut a 64-bit pointer in half (texSelect faulted on exactly that).
 */
...
#define	K1_TO_PHYS(x)	((uintptr_t)(x))	/* kseg1 to physical */
#define	KDM_TO_PHYS(x)	((uintptr_t)(x))	/* direct mapped to physical */
#define	PHYS_TO_K0(x)	((void *)(uintptr_t)(x))	/* physical to kseg0 */
#define	PHYS_TO_K1(x)	((void *)(uintptr_t)(x))	/* physical to kseg1 */
