...
/*
 * The controller-pak layer belongs to libultra, and this port supplies it in
 * port/src/libultra.c, which defines the whole osPfs* family together. Keeping
 * this copy as well is a duplicate definition at link time.
 */
#if 0
...
void joyPoll(void)
{
    ...
    if (g_ContInitDone && osRecvMesg(&g_ContInputMessageQueue, &msg, 0) == 0)
    {
        ...
        {
            if (((g_ContData[0].samples[g_ContData[0].nextlast].pads[i_s8].errnum == 0) &&
                 (g_ContData[0].samples[g_ContData[0].nextsecondlast].pads[i_s8].errnum != 0)) ||
                ((g_ContData[0].samples[g_ContData[0].nextlast].pads[i_s8].errnum != 0) &&
                 (g_ContData[0].samples[g_ContData[0].nextsecondlast].pads[i_s8].errnum == 0)))
            {
                joyCheckStatus();
                break;
...
s8 joyGetStickX(s8 contpadnum)
{
     ...
     * past its array; here that write landed on the controller callback
     * slots that follow it. Such a pad simply reads as nothing.
     */
    if (contpadnum < 0 || contpadnum >= MAXCONTROLLERS)
    {
        return 0;
    }
