...
#ifdef GEVR
    /*
     * There is no MIPS address segmentation here, and the heap is an ordinary
     * allocation, so its base is already the address to use. Going through
     * PHYS_TO_K0(osVirtualToPhysical(...)) would not: PHYS_TO_K0 is
     * ((u32)(x)|0x80000000) in C, which truncates a 64-bit heap base to its low
     * word. Every pool built from it then pointed into nothing.
     */
...
void bossMainloop(void)
{
    ...
    s32 freeGfx;
    s32 mainTickElapsed;
    uintptr_t rspReplyMsg; /* an address; as s32 it lost its top half and the reply landed nowhere */
    ...
    while (!done)
    {
        ...
        debmenuRefresh();
        waitForNextFrame();
        sysLogPrintf(LOG_NOTE, "stage: loading: first frame waited");
    ...
    */
                    ...
                    else
                    {
                        if (g_MainStageNum < 0 && pendingGfx < 2U)
                        {
                            ...
                            rspReplyMsg = (uintptr_t)(&localGfxDoneMsg);
        ...
        sysLogPrintf(LOG_NOTE, "stage: unloaded stage text data");
        stop_demo_playback();
