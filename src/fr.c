...
{
    /* PORT: the renderer maps 2D rectangles through its "native viewport",
       which nothing here updated (it booted at 320x220); the 440x330 front-end
       screens were drawn 1.375 times too large and cut off. */
    extern void videoUpdateNativeResolution(s32 w, s32 h);

    g_ViBackData->x = x;
    g_ViBackData->y = y;
    videoUpdateNativeResolution(x, y);
}
