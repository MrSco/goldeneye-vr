...
#ifdef GEVR
#include "system.h"
#define GEVR_PTR(p) ((uintptr_t)(p))
#else
#define GEVR_PTR(p) ((s32)(p))
#endif
...
Acmd *alAdpcmPull(void *filter, s16 *outp, s32 outCount, s32 sampleOffset, Acmd *p) 
{
    ...
    if (looped){
        ...
        f->memin = GEVR_PTR(f->table->base) + ADPCMFBYTES *
     ...
     */
    overFlow = f->memin + nbytes - (GEVR_PTR(f->table->base) + f->table->len);
    if (overFlow < 0)
        overFlow = 0;
...
Acmd *alRaw16Pull(void *filter, s16 *outp, s32 outCount, s32 sampleOffset, Acmd *p) 
{
    ...
    if ((outCount + f->sample > f->loop.end) && (f->loop.count != 0)){
        ...
        if (nSam > 0){
            ...
            #ifdef GEVR
            aLoadBufferRaw16Impl((const void *)(dramLoc - dramAlign));
            ptr++;
#else
        ...
        *outp += dramAlign;
        ...
        f->memin = GEVR_PTR(f->table->base) + (f->loop.start<<1);
        f->sample = f->loop.start;
        op = *outp;
