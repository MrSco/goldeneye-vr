...
#ifdef GEVR
    /* Cartridge cseq headers are big-endian; convert once on open. */
    if (seq->base->division > 0xff) {
        s32 j;
        for (j = 0; j < 16; j++) {
            u32 v = seq->base->trackOffset[j];
            seq->base->trackOffset[j] =
                ((v & 0xff) << 24) | ((v & 0xff00) << 8) |
                ((v >> 8) & 0xff00) | (v >> 24);
        }
        {
            u32 d = seq->base->division;
            seq->base->division =
                ((d & 0xff) << 24) | ((d & 0xff00) << 8) |
                ((d >> 8) & 0xff00) | (d >> 24);
        }
    }
#endif
