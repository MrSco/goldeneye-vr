...
void alSynNew(ALSynth *drvr, ALSynConfig *c)
{
     ...
     */
#ifdef GEVR
    _Static_assert(sizeof(ALParam) >= sizeof(ALStartParamAlt), "audio parameter pool slot too small");
    _Static_assert(sizeof(ALParam) >= sizeof(ALStartParam), "audio start parameter pool slot too small");
#endif
