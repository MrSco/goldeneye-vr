...
#ifdef GEVR
/*
 * N64 used 0x2E000 for 32-bit AL* layouts. On 64-bit hosts the same object
 * counts need more space, and MEMPOOL_PERMANENT is too small, so the AL heap
 * comes from sysMemAlloc instead of memp.
 */
#define MUSIC_ALLOCATION_BYTES   0x100000
#else
...
void musicSeqPlayerInit(void)
{
...
#ifdef GEVR
    u8 *tblSegmentRomStartAddress = _musicsampletblSegmentRomStart;
#else
...
#ifdef GEVR
    p = (u8 *)sysMemZeroAlloc(MUSIC_ALLOCATION_BYTES);
    if (!p) {
        sysFatalError("music: could not allocate %u byte AL heap", (u32)MUSIC_ALLOCATION_BYTES);
    }
#else
    ...
    {
#ifdef GEVR
        /* Already host-endian from gevrRomBindMusic; do not copy into the AL heap. */
        sfxBank = (ALBankFile *)_sfxctlSegmentRomStart;
        alBnkfNew(sfxBank, _sfxtblSegmentRomStart);
#else
    ...
    {
#ifdef GEVR
        instrumentBank = (ALBankFile *)_instrumentsctlSegmentRomStart;
        alBnkfNew(instrumentBank, _instrumentstblSegmentRomStart);
#else
...
#ifdef GEVR
    /*
     * Cartridge entries are {u32 offset, u16 raw, u16 packed} (8 bytes). On
     * 64-bit hosts RareALSeqData has an 8-byte pointer, so a blind romCopy
     * misaligns every field. Expand into host layout here.
     */
    {
        u16 seqCount;
        u32 i;
        u8 *romTbl = _musicsampletblSegmentRomStart;
        u32 hostSize;

        seqCount = *(u16 *)romTbl;
        hostSize = 4 + (u32)seqCount * (u32)sizeof(RareALSeqData);
        g_musicDataTable = alHeapAlloc(&g_musicHeap, 1, ALIGN16_a(hostSize));
        g_musicDataTable->seqCount = seqCount;
        g_musicDataTable->unk = 0;
        for (i = 0; i < seqCount; i++) {
            u8 *e = romTbl + 4 + i * 8;
            u32 off = (u32)e[0] | ((u32)e[1] << 8) | ((u32)e[2] << 16) | ((u32)e[3] << 24);
            u16 raw = (u16)(e[4] | (e[5] << 8));
            u16 packed = (u16)(e[6] | (e[7] << 8));
            g_musicDataTable->seqArray[i].address = romTbl + off;
            g_musicDataTable->seqArray[i].uncompressed_len = raw;
            g_musicDataTable->seqArray[i].len = packed;
        }
        /* addresses already absolute; skip musicSeqFileNew */
    }
#else
...
void musicTrack1Play(s32 track)
{
    ...
    decompressdata(temp_a0, thing.seqData, (struct huft *)hlist);
