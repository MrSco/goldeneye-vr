...
void mempCheckMemflagTokens(uintptr_t poolAreaStart, s32 poolAreaSize)
{
    ...
    /*
     * poolAreaStart is an address, so it is pointer width. It used to be s32,
     * which on the N64 was the whole of RAM but here silently cut the heap base
     * down to its low word - every pool then started somewhere that was not
     * mapped, and the first allocation handed out a pointer that faulted on use.
     * The bank offsets below stay s32: those are sizes, not addresses.
     */
...
void mempSetBankStarts(s32 poolSizes[MEMPOOL_COUNT+1])
{
    ...
    s32 mempLen;
    s32 mempRequested;
    uintptr_t mempStart;   /* an address; bankstarts[] beside it are offsets */
...
void *mempAllocBytesInBank(u32 bytes, u8 poolnum)
{
    ...
    if (pool->pos == NULL)
    {
        sysFatalError("memp: bank %d has no memory (pos NULL), %u bytes requested", poolnum, bytes); /* PORT: was a silent spin */
    }
    ...
    if (pool->pos > pool->end)
    {
        ...
        sysFatalError("memp: bank %d overrun before a %u-byte request", poolnum, bytes);
    }
    ...
    if (pool->pos + bytes > pool->end)
    {
        ...
        sysFatalError("memp: bank %d out of memory: %u bytes requested, %ld free (bank is %ld bytes)", poolnum, bytes, (long)(pool->end - pool->pos), (long)(pool->end - pool->start));
    }
...
MEMP_ADD_ENTRY_RESULT mempAddEntryOfSizeToBank(void *allocation, s32 newsize, u8 poolnum)
{
    ...
    if (pool->pos == 0)
    {
        sysFatalError("memp: grow in bank %d with no memory", poolnum);
    }
    ...
    {
        nulled_mempLoopAllMemBanks();
        sysFatalError("memp: grow in bank %d after an overrun", poolnum);
    }

    if (pool->pos + growsize > pool->end)
    {
        nulled_mempLoopAllMemBanks();
        sysFatalError("memp: grow in bank %d out of memory: %d more bytes, %ld free", poolnum, growsize, (long)(pool->end - pool->pos));
    }
...
// Returns the allocation, so it is pointer width - it used to return u32,
// which cut the pointer in half here the same way the pool bases were cut.
