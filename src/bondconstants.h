...
{
    RUNTIMEBITFLAG_NONE,
    RUNTIMEBITFLAG_00000001_VAL           = 1 << 0,
    RUNTIMEBITFLAG_00000002_VAL           = 1 << 1,
    RUNTIMEBITFLAG_REMOVE                 = 1 << 2, /* removes object when set                                         */
    RUNTIMEBITFLAG_ISRETICK               = 1 << 3,
    RUNTIMEBITFLAG_TAGGED                 = 1 << 4,
    RUNTIMEBITFLAG_THROWING_KNIFE_RELATED = 1 << 5,
    RUNTIMEBITFLAG_EMBEDDED               = 1 << 6,
    RUNTIMEBITFLAG_HASPROJECTILE          = 1 << 7, /* this object owns a live Projectile record in obj->projectile   */
    RUNTIMEBITFLAG_00000100_VAL           = 1 << 8,
    RUNTIMEBITFLAG_BEENOPENED             = 1 << 9,
    RUNTIMEBITFLAG_DESTROYED              = 1 << 10, /* only set with disabled or destroyed doors                    */
    RUNTIMEBITFLAG_00000800_VAL           = 1 << 11,
    RUNTIMEBITFLAG_00001000_VAL           = 1 << 12,
    RUNTIMEBITFLAG_PADLOCKEDDOOR_BIT      = 1 << 13,
    RUNTIMEBITFLAG_ACTIVATED              = 1 << 14, /* activated                                                    */
    RUNTIMEBITFLAG_00008000_VAL           = 1 << 15,
    RUNTIMEBITFLAG_00010000_VAL           = 1 << 16,
    RUNTIMEBITFLAG_00020000_VAL           = 1 << 17,
    RUNTIMEBITFLAG_00040000_VAL           = 1 << 18,
    RUNTIMEBITFLAG_HASOWNER_BIT           = 1 << 19
} RUNTIMEBITFLAG;
...
#define RUNTIMEBITFLAG_00008000       0x00008000
#define RUNTIMEBITFLAG_00010000       0x00010000
...
{
    /* Save queries return -1 for an empty folder and scan backwards past
     * Dam. Keep the enum signed on native compilers, or those scans wrap
     * from zero to UINT_MAX and never finish. Existing mission ids stay put. */
    SP_LEVEL_NONE = -1,
    SP_LEVEL_DAM,
    SP_LEVEL_FACILITY,
    SP_LEVEL_RUNWAY,
...
#pragma region Object Instance Stuff
    typedef enum BODIES
    {
        ...
        BODIES_MAX,
        /*
         * PORT: the cast table in front.c ends with a body of 0xFFFFFFFF and
         * tests `body < 0` for it. The cartridge compiler gave every enum a
         * signed int; clang makes one unsigned unless a member is negative.
         */
        BODY_TABLE_END = -1
    } BODIES;
    ...
    typedef enum HEADS
    {
        ...
        /*
         * PORT: written as 0xFFFFFFFF / 0xFFFFFF9F on the cartridge, where the
         * compiler still gave the enum a signed int. Here those constants do
         * not fit in an int, the enum becomes unsigned, and every `head >= 0`
         * test that guards the head load passes for HEAD_FIXED. The values are
         * the same bit patterns, spelled as the negative numbers they are.
         */
        HEAD_FIXED        = -1,
        HEAD_RANDOM       = -97
    } HEADS;
...
#define PROPFILERECORD(NAME, SCALE) \
    {&NAME##_header, STR(P##NAME##Z), (SCALE)},
