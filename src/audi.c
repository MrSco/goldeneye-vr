...
#ifdef GEVR
uintptr_t amDmaCallback(uintptr_t addr, s32 len, void* state);
#else
...
/*
 * Threads are stubs on this port, so amMain never runs. The frame pump calls
 * this once per retrace instead - running the stock GE synthesizer and submitting its PCM to SDL.
 */
static s32 gevrAudioPumpArmed;
static u32 gevrAudioRetraces;

void gevrAudioMarkReady(void)
{
	gevrAudioPumpArmed = 1;
	gevrAudioRetraces = 0;
}
...
void gevrAudioFrame(void)
{
	...
	amHandleFrameMessage(info, NULL);

	if (osRecvMesg(&g_AudioManager.replyMessageQueue, (OSMesg *)&done, OS_MESG_NOBLOCK) == 0 && done) {
		amHandleDoneMessage(done);
        /* Soft execution is complete; do not hold PCM for another retrace pair. */
        osAiSetNextBuffer(done->data, done->frameSamples * 4);
	}
...
void amHandleFrameMessage(AudioInfo *info, AudioInfo *lastInfo)
{
...
#ifdef GEVR
    /* SDL consumes blocks independently of XR. Keep three audio frames queued
     * (~100 ms) so a late retrace doesn't insert silence. Use signed arithmetic
     * and cap synthesis to the actual output allocation. */
    s32 queuedSamples = (s32)(osAiGetLength() >> 2);
    s32 wantedSamples = (s32)(3 * g_FrameSize) - queuedSamples;
    if (wantedSamples < (s32)g_MinFrameSize) wantedSamples = (s32)g_MinFrameSize;
    if (wantedSamples > (s32)g_MaxFrameSize) wantedSamples = (s32)g_MaxFrameSize;
    info->frameSamples = (s16)(wantedSamples & ~0xf);
#else
...
#ifdef GEVR
    if (info->frameSamples > g_MaxFrameSize) {
        sysFatalError("audio output overflow samples=%d max=%u", info->frameSamples, g_MaxFrameSize);
    }
#endif
    ...
    info->task.list.t.data_size = (cmdlp - g_AudioManager.cmdList[g_CurrentAcmdList]) * sizeof(Acmd);
...
#endif
{
    ...
    static u64 silence[(AUDIO_DMA_MAX_BUFFER_LENGTH + 16) / sizeof(u64)];
    /* Bound the request before doing address arithmetic or looking up a cache. */
    if (!g_RomFile || len <= 0 || len > AUDIO_DMA_MAX_BUFFER_LENGTH - 8 ||
        addr < (uintptr_t)g_RomFile ||
        addr - (uintptr_t)g_RomFile >= g_RomFileSize) {
        sysLogPrintf(LOG_NOTE, "amDma: reject addr=%p len=%d", (void *)addr, len);
        return (uintptr_t)silence;
    }
...
#ifdef GEVR
                 && buffEnd - addrEnd >= 8 /* decoder rounds DMA loads up */
#endif
        )
        {
            /* mark it used */
...
#ifdef GEVR
    /* DMA is synchronous on the host. Zero-pad a final partial ROM block. */
    size_t remaining = g_RomFileSize - (addr - (uintptr_t)g_RomFile);
    size_t count = remaining < AUDIO_DMA_MAX_BUFFER_LENGTH ? remaining : AUDIO_DMA_MAX_BUFFER_LENGTH;
    memset(freeBuffer, 0, AUDIO_DMA_MAX_BUFFER_LENGTH);
    memcpy(freeBuffer, (const void *)addr, count);
#else
    osPiStartDma(&g_DmaIOMessageBuffer[g_NextDMa++], OS_MESG_PRI_HIGH, OS_READ, addr, freeBuffer, AUDIO_DMA_MAX_BUFFER_LENGTH, &g_DmaMessageQueue);
#endif
