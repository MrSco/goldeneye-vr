...
struct Model *makeonebody(s32 body, s32 head, struct ModelFileHeader *bodyHeader, struct ModelFileHeader *headHeader, s32 sunglasses, struct Model *model)
{
    f32 scale;
    f32 pov;
    ModelNode *headnode; /* the head placeholder node; was an s32 named opcode holding its address, cut in half */
    ...
    {
        headnode = bodyHeader->Switches[4];
        if (headnode != NULL)
        {
  ...
  //
    ...
    if (model != 0)
    {
        ...
        {
            bodyHeader->numRecords -= headHeader->numRecords;
            modelAttachHead(model, headnode, headHeader);
