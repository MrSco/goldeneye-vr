...
void select_ramrom_to_play(void)
{
...

    {
         ...
         * ends.
         */
        ramromfilestructure *demo = ramrom_table[randomGetNext() % i].fdata;

        if (*(void **)demo == NULL)
        {
            sysLogPrintf(LOG_WARNING, "ramrom: demo data not bound; returning to the title instead of playing it");
            bossRunTitleStage();
            return;
        }

        replay_recorded_ramrom_at_address(demo);
    }
}
