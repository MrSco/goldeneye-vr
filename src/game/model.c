...
Model *modelmgrInstantiateModel(ModelFileHeader *header)
{
    ...
    if (g_ModelIsLvResetting) 
    {
        ...
        if (model == NULL) 
        {
            model = mempAllocBytesInBank(sizeof(struct ModelSlot), MEMPOOL_STAGE); /* the bare model head; 0x20 on the cartridge */
        }
    ...
    else 
    {
        ...
        if (model == NULL)
        {
            /* PORT: see modelmgrInstantiateModelWithAnim - the same safety net. */
            sysLogPrintf(LOG_WARNING, "model: no free slot for %d words of rwdata (%d slots); allocating one",
                    header->numRecords, g_MaxModelSlots);
...
Model *modelmgrInstantiateModelWithAnim(ModelFileHeader *modelFileHeader)
{
    ...
    if (g_ModelIsLvResetting) 
    {
        ...
        if (newModel == NULL) 
        {
            newModel = mempAllocBytesInBank(sizeof(Model), MEMPOOL_STAGE); /* a whole animated model; 0xc0 on the cartridge */
        }
    ...
    else 
    {
        ...
        {
            /*
             * PORT: no spare slot can hold this model's runtime data. On the
             * cartridge that never happened; here the records are bigger. Take a
             * fresh model rather than hand back NULL, and say so.
             */
            sysLogPrintf(LOG_WARNING, "model: no free animated slot for %d words of rwdata (%d slots); allocating one",
                    requiredRwdatalen, g_MaxAnimModelSlots);
            newModel = mempAllocBytesInBank(sizeof(Model), MEMPOOL_STAGE);
            if (requiredRwdatalen > 0)
            {
...
void set_vtxallocator(struct Vertex *(*param_1)(s32)) { /* was s32: a function pointer, cut in half */
  vtxallocator = param_1;
}


// called after a debug print during failed model operation possible "exit()" function in debug
// Not behind LEFTOVERDEBUG: bondview2.c, initBondDATAdefaults.c and
// objecthandler.c call it unconditionally.
...
{
    s32 index  = 0;
    u32 *data = Objinst->datas; /* 4-byte units, like the indices */
...
u16 modelAnimReadRootMotionValue(ModelAnimation *anim, s32 fieldIndex, s32 extraBitOffset)
{
    ...
    result = 0;
    desc = modelAnimBitDescriptors(anim) + fieldIndex;
    ...
    if (bitsRemaining > 0)
    {
        ...
        totalBitOffset &= 7;
        byteptr = modelAnimBitStream(anim) + byteIndex;
...
void sub_GAME_7F06DB5C(ModelRenderData *arg0, Model *arg1, ModelNode *arg2, quatf arg3)
{
    ...
    f32 sp24;
    s32 _gap20;
    ModelNode *parent;   /* PORT: was an s32 stack slot holding a pointer */
    Mtxf *jointmtx;
    ...
    if (parent != NULL) {
        sp9C = modelFindNodeMtx(arg1, parent, 0);
    } else {
        sp9C = arg0->basemtx;
    }
    ...
    if (sp9C != 0) {
        ...
        jointmtx = (Mtxf *)&sp48[sp54];
        matrix_4x4_multiply_homogeneous(sp9C, &sp58, jointmtx);
        if (g_ModelJointPositionedFunc != NULL) {
            g_ModelJointPositionedFunc(sp54, jointmtx);
        }
    } else {
...
void sub_GAME_7F07306C(ModelRenderData *param_1, /* was s32: a pointer, cut in half */struct Model *param_2,struct ModelNode *param_3)
{
    return;
}
...
void sub_GAME_7F0737EC(ModelRenderData *param_1, /* was s32: a pointer, cut in half */struct Model *param_2, struct ModelNode *param_3)
{
    return;
}


void sub_GAME_7F0737FC(ModelRenderData *param_1, /* was s32: a pointer, cut in half */struct Model *param_2,struct ModelNode *param_3)
{
    return;
}
...
void sub_GAME_7F074514(ModelRenderData *param_1, /* was s32: a pointer, cut in half */struct Model *param_2,struct ModelNode *param_3)
{
    return;
}
 ...
 *
 * anim->address is the frame data's offset inside the ANIMATION_ENTRIES
 * segment, exactly as the cartridge stores it. The N64 loader rewrote that
 * slot into an absolute ROM address, and a test on the top bit then told a RAM
 * copy from a cartridge one. Nothing on this port ever produces a RAM-resident
 * animation - no code writes anim->address - and a host pointer would not fit
 * the 4-byte slot anyway, so the offset is left alone and the segment base is
 * added here, on every read.
 *
 * Returns the address of the frame in the scratch buffer, which is a host
 * pointer, hence the width.
*/
uintptr_t loadAnimationFrame(ModelAnimation* anim, s32 frame, ModelSkeleton* unused)
{
    ...
    if (D_80036414 != NULL) // should never be NULL after initAnimationsBuffer is called
    {
        ...
        dest = (u8 *)(((uintptr_t)D_80036414->animBufferPtr2 + 15) & ~(uintptr_t)15);
        ret = dest;

        // Offset of this frame inside the entries segment
        offset = (u32)anim->address + (u32)(frame * frameSize);

        // The cartridge DMA needed an even source, so an odd frame is fetched
        // from one byte earlier and the caller is pointed one byte in. The
        // parity that matters is the cartridge offset's, not the host pointer's.
        if (offset & 1)
        {
            offset--;
            frameSize++;
        ...
        source = _animation_entriesSegmentRomStart + offset;
...
/*
 * PORT: the slot holds a segment-5 offset (0x05000000 | offset) that the
 * converter left in host layout; adding diff = base - 0x05000000 at pointer
 * width lands on the record in memory. As u32 the sum lost the top half.
 */
#define PROMOTE(var) \
    if (var) \
    ...
    while (node)
    {
        ...
        switch (type)
        {
            ...
            case MODELNODE_OPCODE_DLCOLLISION:
                {
                    ...
                    /*
                     * Each collision vertex's LinkedTo used to be promoted here
                     * too. Its slot is four bytes inside a 16-byte vertex, so
                     * it stays a segment-5 offset and is resolved where it is
                     * read (see Vertex in bondtypes.h).
                     */
                    rodata->BaseAddr = (void *)fileramaddr;
                    break;
                }
            ...
            case MODELNODE_OPCODE_OP11:
                {
                    ModelRoData_Op11Record* rodata = &node->Data->Op11;
                    /* unk0c[15] is a 4-byte slot holding a reference; it stays a
                       segment-5 offset (a pointer would not fit). Nothing reads it yet. */
                    rodata->BaseAddr = (void *)fileramaddr;
                    break;
                }
...
{
    /* PORT: drawjointlist tests this flag unconditionally and logs a line per
       joint when it is clear; the cartridge only set it in debug builds. */
