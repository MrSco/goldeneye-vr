...
void bgApplyDynamicCCRMLUT(Gfx *start, Gfx *end, enum CCRMLUT lutIndex)
{
    ...
    /* PORT: the opcode is the top byte of w0, not byte 0 of the command (that was big-endian order) */
    while (((end != NULL) && (curGfx < end)) || ((end == NULL) && ((u8)(curGfx->words.w0 >> 24) != (u8)G_ENDDL)))
    {
