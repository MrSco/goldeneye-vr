...
#include "file2.h"
#include "front.h"
#include "file.h" /* fileGetSaveForFoldernum returns a pointer; without the prototype it was truncated */
...
/*
 * PORT: the wallet-Bond model shares ptr_logo_and_walletbond_DL with the
 * folder screen's display lists and the briefing text, which start right
 * after it. The cartridge gave the model 0xA000 bytes; the host layout is
 * bigger (38016 bytes before the display lists are rewritten with room for
 * their texture loads), so the region is doubled and the neighbours move.
 */
#define WALLETBOND_MODEL_BYTES (4096 * 20)

uintptr_t ptr_menu_videobuffer = 0; /* an address; as s32 it lost its top half */
...
Gfx *display_aligned_white_text_to_screen(Gfx *dl, s32 arg1, s32 arg2, s32 halign, s32 valign, u8 *arg5, struct fontchar *chars, struct font *font);
...
Gfx *display_aligned_white_text_to_screen(Gfx *dl, s32 arg1, s32 arg2, s32 halign, s32 valign, u8 *text, struct fontchar *chars, struct font *font)
{
    ...
    if (text == NULL)
    {
        /* PORT: langGet() had nothing for this slot; it has already said so. */
        return dl;
    }
    ...
    return textRender(dl, &x, &y, text, chars, font, -1, viGetX(), viGetY(), 0, 0);
}
...
Gfx *constructor_menu00_legalscreen(Gfx *DL)
{
    ...
    /* PORT: the original ended at the next global in .data (legalscreen_MRD), which
       only worked because the N64 link placed it right after the array. */
    legal_text_end = legalpage_text_array + ARRAYCOUNT(legalpage_text_array);
...
void load_walletbond(void)
{
    ...
    if (walletinst[0] == NULL)
    {
        ...
        if (mnode != NULL)
        {
            ...
            arg0 = (Gfx *)((u8 *)srecord->BaseAddr + ((u32)(uintptr_t)srecord->Primary & 0xffffff)); /* PORT: was s32 arithmetic */
...
{
    s32 size = 0x78000 - WALLETBOND_MODEL_BYTES; /* PORT: what is left of the 0x78000 allocation after the model */
    Gfx* DL = (Gfx *)(ptr_logo_and_walletbond_DL + WALLETBOND_MODEL_BYTES); /* PORT: was summed as s32, which cut the pointer in half */
    int i;
...
void load_briefing_text_for_stage(void)
{
    ...
    // what is this
    temp_s0 += argg; /* Byte reservation, independent of host Gfx size. */
