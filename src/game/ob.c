...
#include "system.h"
extern void gevrRomBindFileTable(void);
extern void gevrReportUnboundFile(fileentry *entry, s32 index);
#endif
    ...
    else
    {
...
#ifdef GEVR
        /*
         * A model file is rebuilt in host layout here, while the whole region
        ...
        if (gevrModelPendingHeader != NULL)
        {
            u32 converted = gevrModelConvert(ptrdata, lookupdata->poolRemaining, (u32)bytes,
                    gevrModelPendingHeader->numSwitches, gevrModelPendingHeader->numtextures,
                    srcfile->filename);

            if (converted != 0)
            {
                lookupdata->poolRemaining = converted;
            }
        }
        else if (srcfile->filename != NULL && strncmp(srcfile->filename, "Tbg_", 4) == 0)
        {
            size_t converted = gevrConvertStan(ptrdata, lookupdata->poolRemaining, bytes);
            if (!converted) sysFatalError("stage: collision conversion failed for %s", srcfile->filename);
            lookupdata->poolRemaining = converted;
            sysLogPrintf(LOG_NOTE, "stage: converted collision %s (%u host bytes)", srcfile->filename, (u32)converted);
        }
        else if (srcfile->filename != NULL && strncmp(srcfile->filename, "Ubrief", 6) == 0)
        {
            gevrRomSwapBriefing(ptrdata, lookupdata->poolRemaining);
        }
        else if (srcfile->filename != NULL && srcfile->filename[0] == 'U'
                 && (strncmp(srcfile->filename, "Usetup", 6) == 0
                     || strncmp(srcfile->filename, "Ump_", 4) == 0))
        {
            size_t converted = gevrConvertSetup(ptrdata, lookupdata->poolRemaining, bytes);
            if (!converted) sysFatalError("stage: setup conversion failed for %s", srcfile->filename);
            lookupdata->poolRemaining = converted;
            sysLogPrintf(LOG_NOTE, "stage: converted setup %s (%u host bytes)", srcfile->filename, (u32)converted);
        }
        else if (srcfile->filename != NULL && srcfile->filename[0] == 'L')
        {
            /* a text bank (L<stage><language>): its offset table is big-endian */
            gevrRomSwapLangBank(ptrdata, lookupdata->poolRemaining, srcfile->filename);
        }
#endif
#if DEBUG
        if (result == 0)
...
#ifdef GEVR
    /*
     * There is no dev host on this port, so the only honest outcome is "did not
     * load". The original body below walks a payload buffer the host was
     * supposed to have filled, which faults here.
     *
     * Reaching this at all means the ROM loader left hw_address NULL for the
     * file, so report which one - the array index is what identifies it in the
     * manifest.
     */
    gevrReportUnboundFile(srcfile, (s32)(srcfile - file_resource_table));
...
#ifdef GEVR
    /*
     * Fill hw_address and rom_size from the player's cartridge. The loop above
     * derives sizes from the gap to the next entry, which does not hold here:
     * the table is shared across regions and a US ROM has no data for the
     * PAL-only levels, so the loader writes both fields itself.
     */
