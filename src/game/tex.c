...
s32 texLoadFromGdl(Gfx *src, s32 srcsize, Gfx *dst, void *texpool)
{
    ...
    count = srcsize / (s32)sizeof(Gfx); /* a command is 16 bytes here, not 8 */
    ...
    while (count > 0)
    {
        switch ((u8)(in->words.w0 >> 24)) /* the opcode is the top byte of w0, not the first byte in memory */
        {
            case G_NOOP:
                if (!syncEmitted)
                {
...
{
    /* whole commands, back to front so the ranges may overlap */
    arg2 = arg2 / (s32)sizeof(Gfx);
    arg0 = arg0 + (arg2 - 1);
    arg1 = arg1 + (arg2 - 1);
