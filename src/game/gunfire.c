...
Gfx* watchRenderController(Gfx* gdl, Mtxf* basemtx, s32 envcolour, bool animatebuttons, WatchContButtonPositions* buttonpositions, s8* contpadnum)
{
    ...
    {
        matrix_4x4_copy((Mtxf *)((u8 *)modelstack.render_pos + i * sizeof(Mtxf)), &sp41c); /* was (u32): the watch controls page crashed on the cut pointer */
...
void gunTickHandState(enum GUNHAND hand, s32 triggerOn)
{
    ...
    f32 temp_f0_2;
    Weapon1PTransformKeyframe *var_a0_2; /* a keyframe table; as u32 the pointer was cut in half */
    f32 temp_v1_9;
...
void sub_GAME_7F068508(GUNHAND handnum, f32 floor_y_pos)
{
    ...
    weaponid = getCurrentPlayerWeaponId(handnum);
#ifdef GEVR
    /* the pistol branch sets vel.z = frac * 0.0f before frac is assigned:
     * stack garbage that is NaN or Inf made the whole casing NaN, and a NaN
     * casing never falls below its floor, so it held its slot for good */
    frac = 0.0f;
#endif
...
#if defined(VERSION_JP) || defined(VERSION_EU) || defined(BUGFIX_R1)
		jp_hudmsgBottomShow(buffer);
#else
		hudmsgBottomShow(buffer);
#endif
