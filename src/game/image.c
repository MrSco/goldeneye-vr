...
/*
 * M-113 / M-114 (gepc-ref): the decoder and the renderer disagree about the
 * byte order of the texpool buffer, and the split is keyed on the GBI texture
 * SIZE, which corresponds 1:1 to the pixel format decoded at each call site
 * below. Verified against this port's own importers, not assumed:
 *
 *   G_IM_SIZ_16b (RGBA16, RGB15-via-RGBA, IA16): import_texture_rgba16 reads
 *   the pool as manually big-endian bytes ((addr[0] << 8) | addr[1]), and
 *   import_texture_ia16 takes intensity = addr[0], alpha = addr[1]. A native
 *   u16 store on this little-endian host is therefore read back byte-swapped,
 *   so the decoder must store bswap16(value). Without it the fire and smoke
 *   particles (IMAGE_FIRE_0..14) render blue/purple.
 *
 *   G_IM_SIZ_32b (RGBA32, RGB24): import_texture_rgba32 does PD_BE32() on a
 *   NATIVE u32 load, so it expects the pool to hold native-order words. A
 *   store-time swap inverts those instead - that was M-114 in the reference
 *   (muzzle flash blue, ammo HUD pink), so the 32-bit path stays native.
 *
 ...
 * TEXFORMAT_*, so the scoping lives here. Per the reference's ROM census the
 * non-zlib 16-bit wide-pixel images are IMAGE_FIRE_0..14 plus texnums
 * 1198-1201 (RGBA16), 2430 (IA16) and 2510-2523 (RGB15); every ammo, flare,
 * crosshair and muzzle-flash wide-pixel image is RGBA32.
 *
 * Known limitation, shared with the reference: texShrinkNonPaletted() blends
 * 16-bit pixels as native u16, so mip levels generated from a swapped pool
 * blend wrong. gepc-ref does not guard it either; left alone deliberately.
 */
#ifdef GEVR
...
#endif
#define PORT_PIXEL32(x) (x) /* the 32b importer expects native words - see above */


// bss
//8008C720
/*
 * The default texture pool. Every user takes its address and treats it as a
 * struct texpool, and texInitPool() writes all four fields through that
 * address - so it is the struct, not a pointer to one. As a pointer it was
 * 8 bytes holding a 32-byte struct, and the overflow landed in the globals
 * beside it.
 */
...
{
    /*
     * Walks the list one command at a time. The original scanned bytes for
     * the big-endian pattern fd...... abcd...., which only holds for the
     * cartridge's 8-byte commands; here a command is two host words, so the
     * opcode is the top byte of w0 and the 0xabcd tag the top half of w1's
     * low word.
     */
    while ((u8)(gdl->words.w0 >> 24) != (u8)G_ENDDL)
    {
        // Look for a SETTIMG whose image address is still an 0xabcdXXXX image id
        if ((u8)(gdl->words.w0 >> 24) == G_SETTIMG && ((gdl->words.w1 >> 16) & 0xffff) == 0xabcd)
        {
            texLoad(&gdl->words.w1, arg1);
        }
 ...
 */
/*
 * updateword is the slot holding the image id: a display list's SETTIMG w1,
 * or an sImageTableEntry / ModelFileTextures record's first field. The
 * decoded texture's address is written back into it, so the slot is pointer
 * width.
 */
void texLoad(uintptr_t *updateword, struct texpool *pool)
{
    u8 compbuffer[4000];
    u8 *compptr;
    s32 sp14a8;
    ...
    /*
     * PORT: the slot is either an image id (abcdxxxx or 0000xxxx) or, once
     * this function has run on it, the decoded texture's address. The
     * cartridge told them apart by the kseg0 bit; here anything that is not
     * one of the two id forms is an address, and there is nothing to do.
     * texReset() runs on every stage load over the same static tables, so
     * the second title screen hit this with a pointer's low half as an id.
     */
    if (*updateword > 0xffffffffu || (((*updateword >> 16) & 0xffff) != 0 && ((*updateword >> 16) & 0xffff) != 0xabcd))
    {
        return;
    }
    ...
    if (tex == NULL)
    {
        alignedcompbuffer = (u8 *) (((uintptr_t)compbuffer + 0xF) & ~(uintptr_t)0xF); /* stack address: pointer width, not u32 */
        ...
        if (TRUE)
        {
            ...
            if ((!iszlib && (texFreeBytesInBuffer(pool) < 0x10CC)) || (iszlib && texFreeBytesInBuffer(pool) < 0xA28)) {
                /* PORT: say so; the cartridge did this silently and the texture showed as garbage */
                sysLogPrintf(LOG_WARNING, "texLoad: pool %p full (%d bytes left), texture %d gets garbage", (void *)pool, texFreeBytesInBuffer(pool), g_TexNumToLoad);
...
void texLoadFromModelFileHeader(ModelFileHeader* arg0, struct texpool* arg1)
{
    ...
    for (i = 0; i < arg0->numtextures; i++)
    {
        ...
        {
            texLoad(&textures[i].TextureID, arg1);
        }
    }
}
...
{
    uintptr_t texturenumcopy;
    texturenumcopy = (u32)texturenum;
