...
{
    if (tileStack == NULL || *tileStack == NULL) {
        return 0;
    }
...
void stanDetermineEOF(struct StanPrefixRecord *file /* canonically r */, s32 origBase, u8 *newBase)
{
    ...
    stan_prefix = file;
    if (file == NULL || file->ptr_firstroom == NULL)
    {
        return;
    }

    /*
     * On host, gevrConvertStan has already unpacked and byte-swapped all rooms
     * and tiles into host layout and populated the room table with host pointers.
     * Tile relative links use (link << 3) relative to firstroom - 0x80.
     */
    standTileStart = (StandTile *)(((u8 *)file->ptr_firstroom) - 0x80);
    ptr_firstroom_0 = (uintptr_t)file->ptr_firstroom;

    tile = file->ptr_firstroom;
    while (*(u32 *)tile != 0)
    {
        stanTileEnd = tile;
