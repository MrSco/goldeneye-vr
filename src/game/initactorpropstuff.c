...
s32 initResolveAnimGroupTable(struct weapon_firing_animation_table *animconfig)
{
    ...
    if (animconfig->anim.offset != 0)
    {
        ...
        do
        {
            /* ptr_animation_table is a heap pointer: adding through s32 dropped
               its top half and produced an address that faulted on use. */
            config->anim.anim = (struct ModelAnimation *)((uintptr_t)animoffset
...
s32 initResolveAnimTable(struct StruckAnim *entries)
{
    ...
    if (entry->struck_anim != 0)
    {
        do
        {
            ...
            entry++;
            ptr_animation_table_addr = (struct StruckAnim *)(&ptr_animation_table);
            /* entries points at ptr_animation_table; reading it as s32 took only
               the low word of the pointer. */
            entry[-1].struck_anim =
                    (ModelAnimation *)((uintptr_t)ptr_animation_table + address);
        }
...
#define ANIM_PTR(anim) \
    ((ModelAnimation *)((uintptr_t)&anim + ((uintptr_t)ptr_animation_table)))
