...
/*
 * PORT: the spare slots' runtime-data capacity was tuned for the cartridge's
 * record sizes. Records that hold pointers (the collision display list, the
 * head placeholder) are twice as big here, so the capacity is doubled; the
 * allocation is a few hundred bytes per slot.
 */
#define MODEL_SPARE_RWDATALEN      (0x14 * 2)
#define ANIM_MODEL_SPARE_RWDATALEN (0x8c * 2)
