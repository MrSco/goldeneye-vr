...
void load_font_tables(void)
{
    ...
    /*
     * PORT: the cartridge font is big-endian and lays each fontchar out in 24
     * bytes, four of them a pixel-data offset. The host struct is 32 bytes with
     * an 8-byte pointer, so the bytes cannot be copied in place and the offsets
     * added, as the original did here; gevrRomSwapFont() builds the host layout
     * and resolves the pointers.
     */
    len = (u32)((uintptr_t)_fontbankgothicSegmentEnd - (uintptr_t)_fontbankgothicSegmentStart);
	ptrFontBankGothic = gevrRomSwapFont(_fontbankgothicSegmentRomStart, len, MEMPOOL_STAGE);
	...
	ptrFontZurichBold = gevrRomSwapFont(_fontzurichboldSegmentRomStart, len, MEMPOOL_STAGE);
