...
Vtx *lightFindVertexBaseForTri(Gfx *gfx, s32 room_index)
{
    ...
    while (((gfx->words.w0 >> 24) & 0xFF) != G_VTX )
    { 
        gfx--; 
    }

    ret = (Vtx *)gfx->words.w1;

    if (((uintptr_t) ret & 0xFF000000) == 0x0E000000) 
    {
...
void extract_vertex_indices_from_triangle(Gfx* gfx, u32 tri_type, s32* idx1, s32* idx2, s32* idx3)
{
    ...
    {
        case 0:
            *idx1 = (s32)((gfx->words.w1 >> 16) & 0xFF) / 10;
            *idx2 = (s32)((gfx->words.w1 >> 8) & 0xFF) / 10;
            *idx3 = (s32)((gfx->words.w1 >> 0) & 0xFF) / 10;
            break;
        case 1:
            *idx1 = (s32)((gfx->words.w1 >> 0) & 0xF);
            *idx2 = (s32)((gfx->words.w1 >> 4) & 0xF);
            *idx3 = (s32)((gfx->words.w0 >> 0) & 0xF);
            break;
        case 2:
            *idx1 = (s32)((gfx->words.w1 >> 8) & 0xF);
            *idx2 = (s32)((gfx->words.w1 >> 12) & 0xF);
            *idx3 = (s32)((gfx->words.w0 >> 4) & 0xF);
            break;
        case 3:
            *idx1 = (s32)((gfx->words.w1 >> 16) & 0xF);
            *idx2 = (s32)((gfx->words.w1 >> 20) & 0xF);
            *idx3 = (s32)((gfx->words.w0 >> 8) & 0xF);
            break;
        case 4:
            *idx1 = (s32)((gfx->words.w1 >> 24) & 0xF);
            *idx2 = (s32)((gfx->words.w1 >> 28) & 0xF);
            *idx3 = (s32)((gfx->words.w0 >> 12) & 0xF);
            break;
    }
}
...
void darken_vertex_in_room(Vtx * vertex, s32 room_index)
{
    ...
    vtx_index = (u32)(vertex - (Vtx *)g_BgRoomInfo[room_index].vertices);
...
void lightFixtureBreak(Gfx * hit_gfx, u32 tri_type, s32 room_index)
{
    ...
    for (i = 0; i < LIGHTFIXTURE_TABLE_MAX; i++)
    {
        ...
        {
            if (((fixture_gfx->words.w0 >> 24) & 0xFF) == G_TRI1)
            {
                darken_tri1 = 0;
            ...
            else if (((fixture_gfx->words.w0 >> 24) & 0xFF) == G_TRI4)
            {
                for (j = 0; j < 4; j++)
                {
                    darken_tri4 = 0;
