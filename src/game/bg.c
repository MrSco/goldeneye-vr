...
#include <memp.h>
#include "bg.h"
#include "ob.h" /* _fileNameLoadToBank returns a pointer; without the prototype it was truncated */
#include "bondview.h"
#include "chr.h"
...
//D:80044824
/*
 * sub_GAME_7F0B37EC walks this as a packed byte stream: a level id followed
 * by pairs of portal numbers, terminated by 0xFF, repeated per level.
 *
 * It was written as an array of s_specialportal, whose portallist is a
 * flexible array member. C does not allow those to be initialised, and every
 * element would have a stride of one byte regardless, so the struct form never
 * described this data. Spelling out the byte stream keeps the exact layout the
 * walker reads.
 */
...
void load_bg_file(LEVEL_INDEX levelid)
{
    ...
    size = gevrBgHeaderSize((u8 *)header);
    if (size < 64) sysFatalError("stage: invalid BG header");
    _Static_assert(sizeof(bg_room_data) == sizeof(struct gevrBgRoom), "BG room host layout");
    _Static_assert(sizeof(bg_portal_data_entry) == sizeof(struct gevrBgPortal), "BG portal host layout");
    ptr_bg_data = (uintptr_t) mempAllocBytesInBank(size * 3 + 4096, 4);
    obLoadBGFileBytesAtOffset(levelinfotable[levelentry_index].bg_seg_filename, (u8 *) ptr_bg_data, 0, size);
    if (!gevrConvertBg((u8 *)ptr_bg_data, size, size * 3 + 4096))
        sysFatalError("stage: BG host conversion failed");
    sysLogPrintf(LOG_NOTE, "stage: converted BG metadata %s (%d cartridge bytes)",
            levelinfotable[levelentry_index].bg_seg_filename, size);
    ...
    if (dword_CODE_bss_8007BF98 == 0)
    {
        ...
        if (dword_CODE_bss_8007FF90 != NULL)
        {
            for (i = 0; ((bg_envdata_entry_local *)dword_CODE_bss_8007FF90)[i].type != 0; i++)
            {
                ...
                {
                    ((bg_envdata_entry_local *)dword_CODE_bss_8007FF90)[i].data = getIndexOfPORTALID((uintptr_t) BG_SEG_TO_PTR(ptr_bg_data, ((bg_envdata_entry_local *)dword_CODE_bss_8007FF90)[i].data));
                }
            }
        }
...
s32 getIndexOfPORTALID(uintptr_t portalID)
{
    s32 i;

    for(i = 0; g_BgPortals[i].offset_portal != NULL; i++)
    {
        if (portalID == (uintptr_t)g_BgPortals[i].offset_portal)
        {
            return i;
        }
    }
...
s32 bgLoadRoomVtxData(s32 roomnum, u8 *dst, s32 len)
{
    ...
    */
    offset = (s32)((uintptr_t)ptr_bgdata_room_fileposition_list[roomnum].pPointTableBin & 0x00ffffff);
    obLoadBGFileBytesAtOffset(levelinfotable[levelentry_index].bg_seg_filename, scratch, offset, alignedsize);
    result = bgDecompress(scratch, dst);
    free(scratch);

    if (result > 0) {
        s32 numvtx = result / (s32)sizeof(Vtx);
        Vtx *v = (Vtx *)dst;
        for (s32 i = 0; i < numvtx; i++) {
            v[i].v.ob[0] = (s16)PD_BE16((u16)v[i].v.ob[0]);
            v[i].v.ob[1] = (s16)PD_BE16((u16)v[i].v.ob[1]);
            v[i].v.ob[2] = (s16)PD_BE16((u16)v[i].v.ob[2]);
            v[i].v.flag  = PD_BE16(v[i].v.flag);
            v[i].v.tc[0] = (s16)PD_BE16((u16)v[i].v.tc[0]);
            v[i].v.tc[1] = (s16)PD_BE16((u16)v[i].v.tc[1]);
        }
    }
...
s32 bgLoadRoomPrimaryGdl(s32 roomnum, u8 *dst, s32 allocsize)
{
    ...
    if (size <= 0) {
        roominfo->ptr_expanded_mapping_info = NULL;
        roominfo->usize_primary_DL_binary = 0;
        return 0;
    }
    ...
    fileoffset = (s32)((uintptr_t)ptr_bgdata_room_fileposition_list[roomnum].pPriMappingBin & 0x00ffffff);
    ...
    s32 num_cmds = expanded_size / 8;
    Gfx *unexpanded_host_gdl = malloc((num_cmds + 1) * sizeof(Gfx));
    if (!unexpanded_host_gdl) {
        free(cart_gdl);
        return -1;
    }

    for (s32 i = 0; i < num_cmds; i++) {
        u32 w0 = PD_BE32(*(u32 *)(cart_gdl + i * 8));
        u32 w1 = PD_BE32(*(u32 *)(cart_gdl + i * 8 + 4));
        unexpanded_host_gdl[i].words.w0 = (uintptr_t)w0;
        unexpanded_host_gdl[i].words.w1 = (uintptr_t)w1;
    }
    free(cart_gdl);

    clear_light_fixturetable_in_room(roomnum);

    s32 final_size = texLoadFromGdl(unexpanded_host_gdl, num_cmds * (s32)sizeof(Gfx), (Gfx *)dst, NULL);
    free(unexpanded_host_gdl);

    roominfo->ptr_expanded_mapping_info = dst;
    roominfo->usize_primary_DL_binary = final_size;
...
s32 bgLoadRoomSecondaryGdl(s32 roomnum, u8 *dst, s32 allocsize)
{
    ...
    if (size <= 0) {
        roominfo->ptr_secondary_expanded_mapping_info = NULL;
        roominfo->usize_secondary_DL_binary = 0;
        return 0;
    }
    ...
    fileoffset = (s32)((uintptr_t)ptr_bgdata_room_fileposition_list[roomnum].pSecMappingBin & 0x00ffffff);
    ...
    roominfo->usize_secondary_DL_binary = final_size;
...
void bgLoadRoomModelData(s32 roomID)
{
     ...
     */
    s32 memaRealloc(void *addr, u32 oldsize, u32 newsize);
    ...
    */
    ...
    {
        memaRealloc(data, allocsize, ((used + 0x20) & ~0xf));
    }
...
void bgBuildRoomVtxBounds(s32 roomID)
{
    ...
    maxcmds = g_BgRoomInfo[roomID].usize_primary_DL_binary / (s32)sizeof(Gfx);
    vertices = g_BgRoomInfo[roomID].vertices;
    cmdindex = 0;
    numpoints = 0;

    while ((cmdindex < maxcmds) && ((u8)(gdl[cmdindex].words.w0 >> 24) != (u8)G_ENDDL))
    {
        if ((u8)(gdl[cmdindex].words.w0 >> 24) == (u8)G_VTX) 
        {
            numpoints++;
        }
        cmdindex++;
    ...
    while ((cmdindex < maxcmds) && ((u8)(gdl[cmdindex].words.w0 >> 24) != (u8)G_ENDDL))
    {
        if ((u8)(gdl[cmdindex].words.w0 >> 24) == (u8)G_VTX)
        {
            ...
            numvertices = ((((gdl[cmdindex].words.w0 >> 16) & 0xFF) >> 4) & 0xf) + 1;

            vtx = (Vtx *)(SEGMENT_OFFSET(gdl[cmdindex].words.w1) + (uintptr_t)vertices);
...
bool bgTestRayIntersectionInRoom(coord3d *from, coord3d *to, coord3d *dir, RoomVtxBatchBounds *point, s32 roomnum, struct HitThing *hitthing)
{
    ...
    vtxbase = (Vertex *)((uintptr_t)temp.vertices + SEGMENT_OFFSET(gdl->words.w1));
    temp.roominfo = &g_BgRoomInfo[roomnum];
    bestScore = 0x7FFFFFFF;
    found = 0;
    gdl++;
    op = (u8)(gdl->words.w0 >> 24);

    if ((op != (u8)G_VTX) && (op != (u8)G_ENDDL))
    {
        ...
        do
        {
            if (op == (u8)G_TRI1)
            {
                ...
                bboxMax = D_80044874;
                score = dist;
                idx[0] = (((gdl->words.w1 >> 16) & 0xff) / 10) - vtxoff;
                idx[1] = (((gdl->words.w1 >> 8) & 0xff) / 10) - vtxoff;
                idx[2] = (((gdl->words.w1 >> 0) & 0xff) / 10) - vtxoff;
                i = 0;
                ...
                {
                    if (intersectRayTriangle((Vertex *)((uintptr_t)vtxbase + (idx[0] << 4)),
                                             (Vertex *)((uintptr_t)vtxbase + (idx[1] << 4)),
                                             (Vertex *)((uintptr_t)vtxbase + (idx[2] << 4)),
                                             &ptr_bgdata_room_fileposition_list[roomnum].pos,
                                             from, to, dir, &hitbuf))
                    {
                        tcmd = gdl;
                        ...
                        {
                            void *imgptr = (void *)tcmd->words.w1;
                            if (imgptr != NULL)
                            {
                                struct tex *t = (struct tex *)((u8 *)imgptr - offsetof(struct tex, data));
                                texnum = t->texturenum;
                            }
                            else
                            {
            ...
            else
            {
                if (op == (u8)G_TRI4)
                {
                    ...
                    s2 = 0; do
                    {
                        ...
                        {
                            case 0:
                                idx2[0] = ((gdl->words.w1 >> 0) & 0xf) - vtxoff;
                                idx2[1] = ((gdl->words.w1 >> 4) & 0xf) - vtxoff;
                                idx2[2] = ((gdl->words.w0 >> 0) & 0xf) - vtxoff;
                                break;
                            case 1:
                                idx2[0] = ((gdl->words.w1 >> 8) & 0xf) - vtxoff;
                                idx2[1] = ((gdl->words.w1 >> 12) & 0xf) - vtxoff;
                                idx2[2] = ((gdl->words.w0 >> 4) & 0xf) - vtxoff;
                                break;
                            case 2:
                                idx2[0] = ((gdl->words.w1 >> 16) & 0xf) - vtxoff;
                                idx2[1] = ((gdl->words.w1 >> 20) & 0xf) - vtxoff;
                                idx2[2] = ((gdl->words.w0 >> 8) & 0xf) - vtxoff;
                                break;
                            default:
                                idx2[0] = ((gdl->words.w1 >> 24) & 0xf) - vtxoff;
                                idx2[1] = ((gdl->words.w1 >> 28) & 0xf) - vtxoff;
                                idx2[2] = ((gdl->words.w0 >> 12) & 0xf) - vtxoff;
                                break;
                        }
                        ...
                        {
                            if (intersectRayTriangle((Vertex *)((uintptr_t)vtxbase + (idx2[0] << 4)),
                                                     (Vertex *)((uintptr_t)vtxbase + (idx2[1] << 4)),
                                                     (Vertex *)((uintptr_t)vtxbase + (idx2[2] << 4)),
                                                     &ptr_bgdata_room_fileposition_list[roomnum].pos,
                                                     from, to, dir, &hitbuf))
                            {
                                tcmd = gdl;
        ...
        while ((op != (u8)G_VTX) && (op != (u8)G_ENDDL));
    }
...
bool bgTestBulletHitBackground(coord3d *from, coord3d *to, s32 roomnum, struct HitThing *hit)
{
    ...
    if (found) 
    {
        ...
        if ((u8)(((Gfx*)point)->words.w0 >> 24) != (u8)G_SETTILE) 
        {
        ...
        {
            hit->tileformat = (((Gfx*)point)->words.w0 >> 21) & 0x7;
            hit->tilesize = (((Gfx*)point)->words.w0 >> 19) & 0x3;
        }
    }
