...
void rle_expand_8bit(u8 *src, u8 *dst)
{
    ...
    /*
     * PORT: the header is the cartridge's, big-endian. Read the bytes in that
     * order; as native u16 reads the product came out as a huge count and the
     * expansion ran off the end of the buffer. The run data below is bytes.
     */
    w = (u16)((src[0] << 8) | src[1]);
    h = (u16)((src[2] << 8) | src[3]);
    remaining = w * h;
