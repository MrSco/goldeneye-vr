...
//CODE.bss:80076A50
/*
 * PORT: on the cartridge this label and the dummies that follow it, through
 * g_ModelHitEntriesPenultimate, spanned one pool of 600 twenty-byte hit
 * entries (0x80076A50..0x80079930). Here an entry holds pointers and is 40
 * bytes, so the pool is a real array; threading 600 host entries through a
 * 12-byte label wrote 24000 bytes over the globals that followed it (the
 * player tables among them).
 */
struct ModelHitEntry g_ModelHitEntries[600];
//CODE.bss:80076A5C
...
//CODE.bss:80079908 (g_ModelHitEntriesPenultimate: entry 598 of the pool above)
