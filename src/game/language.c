...
//CODE.bss:8008C640
/*
 * The loaded text banks. Each holds the address the loader returned, so the
 * slots are pointer width; as s32 they kept the low half of the address and
 * langGet() faulted on the remainder.
 */
void *g_LangBanks[45];
...
{
    /*
     * A bank is a table of u32 offsets from the start of the bank, one per
     * slot, followed by the strings. The table arrives big-endian and is
    ...
    {
        sysLogPrintf(LOG_ERROR, "text: slot 0x%X asks for bank %d, which is not loaded", slotID, slotID >> 10);
        return NULL;
    }

    textslot_offset = textbank_ptr[slotID & 0x03FF]; /* load the textbank ptr table then get the slot's offset */

    if (textslot_offset == 0)
    {
        sysLogPrintf(LOG_WARNING, "text: slot 0x%X (bank %d, entry %d) is empty; table starts %08X %08X %08X",
                slotID, slotID >> 10, slotID & 0x3FF, textbank_ptr[0], textbank_ptr[1], textbank_ptr[2]);
    }

    #ifdef DEBUG
    return (textslot_offset != 0) ? (u8 *)textbank_ptr + textslot_offset : "Sorry, string not loaded.";
    #endif
