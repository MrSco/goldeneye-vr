...
s32 memaGrow(uintptr_t addr, u32 amount)
{
    u32        size = 0;
    uintptr_t  new_addr;
    memaspace *curr;
...
void memaDump(void)
{
    ...
    for (curr = &g_MemoryAllocations.spaces[0], tot = 0; curr->addr != (uintptr_t)-1; curr++)
    {
        tot += curr->size;
    }
    ...
    {
        for (curr = &g_MemoryAllocations.spaces[0]; curr->addr != (uintptr_t)-1; curr++)
        {
        ...
        for (curr = &g_MemoryAllocations.spaces[0],acc=0; curr->addr != (uintptr_t)-1; curr++)
        {
            if (curr->size == lim)
            {
                if (count < 200)
...
void mema7000A2F8(void (*func)(uintptr_t, memaspace*)) {
    memaspace *curr = &g_MemoryAllocations.spaces[0];
    while (curr->addr != (uintptr_t)-1) {
 ...
 * US address 7000A3DC.
*/
s32 memaRealloc(void *addr_ptr, u32 oldsize, u32 newsize)
{
