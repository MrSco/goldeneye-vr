                     ...
                     Acmd *p) 
{
	...
	/* ALDelay.input/output are u32 sample offsets; negating them as
	 * unsigned wraps on N64's 32-bit pointers but yields a huge positive
	 * index on LP64 — cast to s32 so delay taps stay behind r->input. */
...
#ifdef GEVR
        ramalign = ((uintptr_t)out_ptr & 0x7) >> 1; /* samples needed to align */
#else
