 ...
 *
 * The retail code encodes that bias in the struct rather than in the index:
 * ALInstrumentAlt_s::soundArray starts at offset 12, exactly one 4-byte
 * pointer ahead of ALInstrument::soundArray at offset 16, so
 * alt->soundArray[id] reads standard entry id - 1. alBnkfNew and
 * preprocessALBankFile both lay the bank out as a standard ALInstrument, so
 * the two views have to stay one slot apart for ids to resolve correctly.
 *
 * With 8-byte pointers the alternate struct's 12-byte header pads out to 16
 * and both arrays start at the same offset, silently making every sound id
 * 0-based - every SFX plays its neighbour in the bank. Derive the correction
 * from the two layouts so it is zero on the N64 and one wherever the padding
 * has eaten the gap.
 */
#define SND_SOUND_INDEX_BIAS \
    (1 - (s32)((offsetof(ALInstrument, soundArray) \
                - offsetof(struct ALInstrumentAlt_s, soundArray)) \
               / sizeof(ALSound *)))
