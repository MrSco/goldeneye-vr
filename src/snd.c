 ...
 * @param soundIndex 1-based index into sound bank:
 * soundBank->instArray[0]->soundArray[soundIndex - SND_SOUND_INDEX_BIAS]
...
#endif
{
    ...
    do
    {
        ...
        sound = (soundBank->instArray[0]->soundArray[soundIndex - SND_SOUND_INDEX_BIAS]);
...
#ifdef GEVR
            /*
             * Rare logo is a composite: id 258 (vol 100, immediate) chains to
             * 259 (vol 60, +467ms) and then 260 (vol 30, +933ms) - the same
             * cymbal sample three times, a semitone apart each, so the layers
             * stack into a rising swell. All three must resolve to bank
             * entries 257/258/259; see SND_SOUND_INDEX_BIAS in snd.h.
             */
            if (soundIndex == 258 || soundIndex == 260 || soundIndex == 259) {
                sysLogPrintf(LOG_NOTE, "sfx chain: id=%d entry=%d vol=%u velMax=%u delay_us=%d pitch=%.4f fx=%d next=%d",
                    (int)soundIndex,
                    (int)(soundIndex - SND_SOUND_INDEX_BIAS),
                    (unsigned)sound->sampleVolume,
                    (unsigned)sound->keyMap->velocityMax,
                    (int)(sound->keyMap->velocityMax * DELTA_33_MS),
                    (double)newState->pitch_28,
                    (int)((sound->keyMap->keyMax & 0xF) * 8),
                    (int)(sound->keyMap->velocityMin + ((sound->keyMap->keyMin & 0xC0) * 4)));
            }
#endif
